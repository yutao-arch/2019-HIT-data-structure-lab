...
//层序遍历算法
int judge_fulltree(btree* T)
{
	if (T->lchild == NULL && T->rchild == NULL) //如果左右儿子都空，则返回1
	{
		return 1;
	}
	if (T->lchild == NULL && T->rchild != NULL) //如果左右儿子只有一个为空，则返回0
	{
		return 0;
	}
	return judge_fulltree(T->lchild) && judge_fulltree(T->rchild); //若左右儿子都存在，递归
}
//判断树是否为完全二叉树
